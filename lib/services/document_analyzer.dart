...
class DocumentAnalyzer {
  ...
  final TextRecognizer _deva = TextRecognizer(script: TextRecognitionScript.devanagiri);

  Future<DocumentVerificationResult> verifyDocument(InputImage image, DocumentType expected) async {
    String text = '';
    try { final r = await _latin.processImage(image); text += ' ${r.text}'; } catch (_) {}
    try { final r = await _deva.processImage(image); text += ' ${r.text}'; } catch (_) {}
    final lower = text.toLowerCase();
    final kws = expected.keywords;
    int hits = 0;
    for (final k in kws) { if (lower.contains(k.toLowerCase())) hits++; }
    final confidence = kws.isEmpty ? 0.0 : hits/kws.length;
    final ok = confidence >= 0.34 || hits >= 2;
  ...
  void dispose() { _latin.close(); _deva.close(); }
}